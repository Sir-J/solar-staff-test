...
export class ExchangeRateMockService {
    ...
    getBaseRate(currency: string, date: Date, rates?: Array<string>): Observable<BaseCurrencyDto> {
        ...
        responseObj.base = currency;
        responseObj.date = moment(date).format('YYYY-MM-DD');
        if (rates && rates.length) {
            const objRates = {};
            rates.forEach((name: string) => (objRates[name] = responseObj.rates[name]));
        }
        return of(this.mapper.deserialize<BaseCurrencyDto>(responseObj, BaseCurrencyDto));
    }
}
